...
static WebServer_StatusTypeDef http_send_response(int32_t socket,
                                                  ...
                                                  uint32_t data_size);
static WebServer_StatusTypeDef http_send(int32_t socket, const char *frame,
                                         uint32_t frame_size);
...
WebServer_StatusTypeDef webserver_http_start(void) {
  /* The IPv4 network socket for this server, to bind with the port to listen
   * to. */
  ...
  net_setsockopt(sock, NET_SOL_SOCKET, NET_SO_BINDTODEVICE, Netif,
                 sizeof(&Netif));
  net_setsockopt(sock, NET_SOL_SOCKET, NET_SO_RCVTIMEO, &timeout,
                 sizeof(timeout));
  ...
  if (net_bind(sock, (struct net_sockaddr *)&s_addr_in, sizeof(s_addr_in)) !=
      0U) {
  ...
  printf("--> Please connect to %s:%" PRIu32 "\n", net_ntoa(&ip_addr_in),
         (uint32_t)NET_NTOHS(s_addr_in.sin_port));
  ...
  while (1) {
    ...
    const int32_t newconn =
        net_accept(sock, (struct net_sockaddr *)&s_addr_in_remote_host,
    ...
    if (newconn > 0) {
      ...
      printf("Request from %s:%" PRIu32 "\n", net_ntoa(&ip_addr_in_remote_host),
             (uint32_t)NET_NTOHS(s_addr_in_remote_host.sin_port));
...
static WebServer_StatusTypeDef http_treat_request(int32_t socket) {
  ...
  if (strncmp((char *)recv_buffer, http_get_cmd, http_get_cmd_size) == 0U) {
    /* Send html */
    if (strncmp((char *)&recv_buffer[http_get_cmd_size], http_html_cmd,
                http_html_cmd_size) == 0U) {
      http_send_response(socket, HTTP_HEADER_HTML_ID, http_header_response,
                         html_buff, html_buff_size);
    } else if (strncmp((char *)&recv_buffer[http_get_cmd_size],
                       http_read_acel_cmd, http_read_acel_cmd_size) == 0U) {
      int x, y;
...
static WebServer_StatusTypeDef http_send_response(int32_t socket,
  ...
  if (http_send_headers_response(socket, headers_id, headers_buff, data_size) !=
      WEBSERVER_OK) {
    return HTTP_ERROR;
  }
...
static WebServer_StatusTypeDef http_send_headers_response(int32_t socket,
  ...
  switch (headers_id) {
    ...
    case HTTP_HEADER_HTML_ID: {
      if (webserver_http_encode_html_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_CSS_ID: {
      if (webserver_http_encode_css_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_JS_ID: {
      if (webserver_http_encode_js_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_FAVICON_ID: {
      if (webserver_http_encode_favicon_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_FONT_ID: {
      if (webserver_http_encode_woff2_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_SENSOR_ID: {
      if (webserver_http_encode_sensor_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_JSON_ID: {
      if (webserver_http_encode_json_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
    ...
    case HTTP_HEADER_IMAGE_ID: {
      if (webserver_http_encode_image_response(headers_buff, data_size) !=
          WEBSERVER_OK) {
        return HTTP_ERROR;
      }
  ...
  if (http_send(socket, (const char *)headers_buff,
                strlen((char *)headers_buff)) != WEBSERVER_OK) {
    return HTTP_ERROR;
  }
...
static WebServer_StatusTypeDef http_send(int32_t socket, const char *frame,
  ...
  while (data_size > 0U) {
    if (data_size >= MAX_SOCKET_DATASIZE) {
      /* Send data */
      if (net_send(socket, (uint8_t *)&frame[data_idx], MAX_SOCKET_DATASIZE,
                   0) <= 0U) {
        return HTTP_ERROR;
      }
