...
function loadData() {
  jQuery.get(tx_url, function (_data, status) {
    ...
    var content = (document.getElementById("tx_active").innerHTML =
      "Resumptions : " + array[0]);
    document.getElementById("tx_suspended").innerHTML =
      "Suspentions : " + array[1];
    document.getElementById("idle_returns").innerHTML =
      "Idle Returns : " + array[2];
    document.getElementById("non_idle_returns").innerHTML =
      "Non Idle returns : " + array[3];
  });
  jQuery.get(nx_url, function (_data, status) {
    var array = _data.split(",");
    document.getElementById("nx_sent").innerHTML =
      "Total Bytes Sent  : " + array[0];
    document.getElementById("nx_received").innerHTML =
      "Total Bytes Received  :  " + array[1];
    document.getElementById("nx_connect").innerHTML =
      "Total connections  : " + array[2];
    document.getElementById("nx_disconnect").innerHTML =
      "Total Disconnections  :  " + array[3];
  });
  jQuery.get(net_url, function (_data, status) {
    var array = _data.split(",");
    document.getElementById("net_ip").innerHTML = "IP ADDRESS : " + array[0];
    document.getElementById("net_port").innerHTML =
      "CONNECTION PORT  :  " + array[1];
  });
